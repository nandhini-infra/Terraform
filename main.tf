locals {
  ...
  log_group_arn = trimsuffix(try(data.aws_cloudwatch_log_group.sfn[0].arn, aws_cloudwatch_log_group.sfn[0].arn, ""), ":*")
   ...
   # Provide a fallback value ("default-role-name") in case both var.role_name and var.name are empty or null

  role_name = local.create_role ? coalesce(var.role_name, var.name, "default-role-name") : null

 # Validate and sanitize the Step Functions state machine name

  valid_name = try(

    regex("^[a-zA-Z0-9_-]{1,80}$", var.name),

    null

  ) != null ? var.name : "default-state-machine-name"
